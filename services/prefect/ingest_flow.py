...
def upsert_csv(table: str, csv_path: str, pk_cols: list[str]):
    ...
    for col in bool_cols:
        if col in df.columns:
            # On convertit en booléen (en supposant des valeurs 0/1 ou True/False)
...
def validate_with_ge(table: str):
    ...
    if table == "users":
        ...
        # On s'assure que user_id n'est pas NULL
        gdf.expect_column_values_to_not_be_null("user_id")

        # Bornes raisonnables : agrégats d'usage >= 0
        gdf.expect_column_values_to_be_between(
            "watch_hours_30d", min_value=0
        )
        gdf.expect_column_values_to_be_between(
            "avg_session_mins_7d", min_value=0
        )
        gdf.expect_column_values_to_be_between(
            "unique_devices_30d", min_value=0
        )
        gdf.expect_column_values_to_be_between(
            "skips_7d", min_value=0
        )
        gdf.expect_column_values_to_be_between(
            "rebuffer_events_7d", min_value=0
        )
...
def ingest_month_flow(seed_dir: str = SEED_DIR, as_of: str = AS_OF):
    ...
    Upsert des tables, puis validation des données avec Great Expectations.
    """
